...
class JsonRPCRouter implements \EF\Routers\IRouter{
    ...
    public function __construct() {
        if ($_SERVER['REQUEST_METHOD'] != 'POST' || empty($_SERVER['CONTENT_TYPE']) || stripos($_SERVER['CONTENT_TYPE'], 'application/json') != 0) {
            throw new \Exception('Require json request', 400);
        }
    }

    public function getMethodMap($ar){
        if (is_array($ar)){
            $this->_map = $ar;
        }
    }
    ...
    public function getURI() {
        if (!is_array($this->_map) || count($this->_map) == 0){
            $ar = \EF\App::getInstance()->getConfig()->rpcRoutes;
